...
router.post('/chat', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(500).json({ reply: 'Agent unavailable. Check your GROQ_API_KEY.', actionPlan: null, savings: null });
    }
    ...
    res.status(500).json({ reply: 'Connection error. Try again!', actionPlan: null, savings: null });
  }
});
