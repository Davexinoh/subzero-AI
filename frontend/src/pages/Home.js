...
import { connectWallet, emergencyFreeze } from '../services/api';
import './Home.css';
...
export default function Home({ leakAmount, setLeakAmount }) {
  ...
  const handleConnectWallet = async () => {
    ...
    if (!address) { setConnectError('Enter a wallet address'); return; }
    if (!/^0x[0-9a-fA-F]{40}$/.test(address)) {
      setConnectError('Invalid address format — must start with 0x and be 42 characters');
      return;
    }
    setConnecting(true);
    setConnectError('');
    try {
      // POST /wallet/connect — WDK reads the address on Base mainnet
      const data = await connectWallet(address);
      setSummary(data);
      setLeakAmount(parseFloat(data.monthlyLeak) || 0);
    } catch (err) {
      const msg = err?.response?.data?.error || 'Could not connect. Check address and try again.';
      setConnectError(msg);
    } finally {
      setConnecting(false);
    }
    ...
    <div className="home-page">
        ...
        <div className="wallet-connect" style={{animation:'slideDown 0.25s ease'}}>
          ...
          <div className="wallet-connect-row">
            <input
              ...
              value={walletInput}
              onChange={e => { setWalletInput(e.target.value); setConnectError(''); }}
          ...
          {summary?.wdkConnected && (
            <p className="wallet-connected-addr">● {summary.walletAddress}</p>
          )}
      ...
      <div className="sys-bar">
        <div className="sys-bar-left">
          <span className={`sys-dot ${mode==='live' && summary?.wdkConnected ? 'sys-dot--live' : ''}`} />
          <span className="sys-label">
            {mode === 'live'
              ? summary?.wdkConnected
                ? summary.walletAddress
                : 'Paste address above to scan'
      ...
      <div className={`leak-section ${frozen?'leak-frozen':''}`}>
        ...
        <div className="status-pills">
          ...
          {mode === 'live' && summary?.wdkConnected && <span className="pill pill--live">● LIVE</span>}
        </div>
      </div>
